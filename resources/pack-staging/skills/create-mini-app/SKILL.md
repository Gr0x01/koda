...
The project also carries an agent guide — one canonical `AGENTS.md` at the project root with `CLAUDE.md` symlinked to it (`ln -s AGENTS.md CLAUDE.md`), so every engine reads the same file. If the project doesn't have it when shaping ends, write it before building: the app in a paragraph, the decisions already settled (with the reason, so they don't get relitigated), constraints, and where things live. It is not the plan doc — the plan is the user's view of what's built and next; the guide is standing orders for whichever agent opens this project cold. When a slice changes something the guide asserts — a settled decision, what exists, where things live — updating the guide is part of finishing that slice.
